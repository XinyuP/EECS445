...
def get_multiclass_training_data(class_size=750):
    ...
    dictionary = project1.extract_dictionary_modified(X_train)
    Y_train = X_train["label"].values.copy()
    X_train = project1.generate_feature_matrix_modified(X_train, dictionary)
...
def get_heldout_reviews(dictionary):
    ...
    dataframe = load_data(fname)
    X = project1.generate_feature_matrix_modified(dataframe, dictionary)
    return X
